...
const Chart: React.FC<ChartProps> = ({
  ...
  const [seriesPolished, setSeriesPolished] = useState([
    name
      ? { name, data: [] as PolishedSeries }
      : { data: [] as PolishedSeries },
  ]);
    ...
    chart: {
      ...
      selection: {
        ...
        xaxis: {
          ...
          max: seriesPolished[0]?.data[
            seriesPolished[0]?.data?.length - 1
          ]?.x?.getTime(),
        },
        fill: {
          color: '#ccc',
  ...
  function getStockOHLCValues(series: StockSeries) {
    ...
    const newTimeSeries = Object.entries(series).reduceRight(
      (newTimeSeries, series) => {
        ...
        if (!isOfTypeBar) {
          newTimeSeries.push({
            x,
            y: [
              Number(series?.[1]?.['open']),
              Number(series?.[1]?.['high']),
              Number(series?.[1]?.['low']),
              Number(series?.[1]?.['close']),
            ],
          });
        } else {
    ...
    const newSeries = seriesPolished.filter(
      (serie) => !(serie?.data && Array.isArray(serie.data)),
    );
